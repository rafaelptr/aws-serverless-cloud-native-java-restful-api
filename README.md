## AWS SAM Application for Store Trips Photos

This is a sample application to demonstrate how to build an application on AWS Serverless Envinronment using the
AWS SAM, Amazon API Gateway, AWS Lambda and Amazon DynamoDB, AWS S3 Bucket.
It also uses the DynamoDBMapper ORM structure to map Trip items in a DynamoDB table to a RESTful API for store trips.
...
2. Create the DynamoDB table. `aws dynamodb create-table --table-name trip --attribute-definitions AttributeName=id,AttributeType=S AttributeName=date,AttributeType=S AttributeName=country,AttributeType=S AttributeName=city,AttributeType=S --key-schema AttributeName=id,KeyType=HASH --billing-mode PAY_PER_REQUEST --endpoint-url http://localhost:8000`
...
invoke the functions rooted at `http://localhost:3000/trips?start=2019-05-01&ends=2020-05-31`.
It shoud return 404. Now you can explore all endpoints, use the src/test/resources/Trips Api.postman_collection.json to import a API Rest Collection into Postman.
